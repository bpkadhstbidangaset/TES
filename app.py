...
def load_master_rak():
    ...
    if not os.path.exists(file_persediaan) or not os.path.exists(file_modal):
        return None, None, f"File '{file_persediaan}' atau '{file_modal}' tidak ditemukan di repository."
...
if f_lra:
    with st.spinner("Sedang memproses dan menyusun data realisasi..."):
        # Baca LRA
    ...
    with m2:
        ...
        <div class="metric-box" style="background: linear-gradient(135deg, #4F46E5, #6366F1);">
            <div class="metric-title">🏢 Total Belanja Modal</div>
    ...
    # ================= TAB 2: BELANJA MODAL =================
    with tab2:
        st.subheader("1. Rekapitulasi per Kategori Aset & Rekening Belanja Modal")
