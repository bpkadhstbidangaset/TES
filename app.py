        ...
        if not df_pers_filtered.empty:
            ...
            if df_sipper_rekap is not None:
                ...
                if round(tot_selisih_p, 2) == 0:
                    st.markdown(f"<div class='status-card-match'><h4 style='color: #065F46; margin:0;'>✅ STATUS: COCOK DENGAN LRA</h4><p style='color: #047857; margin: 4px 0 0 0; font-size:12px;'>Total Realisasi LRA <b>({format_rupiah(tot_lra_p)})</b> sama persis dengan SIPPER <b>({format_rupiah(tot_sipper_p)})</b>. Tidak ada selisih.</p></div>", unsafe_allow_html=True)
                else:
                    st.markdown(f"<div class='status-card-diff'><h4 style='color: #991B1B; margin:0;'>⚠️ STATUS: TERDAPAT SELISIH REKONSILIASI</h4><p style='color: #B91C1C; margin: 4px 0 0 0; font-size:12px;'>Ditemukan selisih sebesar <b>{format_rupiah(tot_selisih_p)}</b> antara LRA ({format_rupiah(tot_lra_p)}) dan SIPPER ({format_rupiah(tot_sipper_p)}).</p></div>", unsafe_allow_html=True)
            else:
                tot_lra_p = rekap_p['Nilai Realisasi'].sum()
                rows_html = "".join([
        ...
        if not df_modal_filtered.empty:
            ...
            if df_modal_app_rekap is not None:
                ...
                if round(tot_selisih_m, 2) == 0:
                    st.markdown(f"<div class='status-card-match'><h4 style='color: #065F46; margin:0;'>✅ STATUS: COCOK DENGAN LRA</h4><p style='color: #047857; margin: 4px 0 0 0; font-size:12px;'>Total Belanja Modal LRA <b>({format_rupiah(tot_lra_m)})</b> sama persis dengan Aplikasi Aset <b>({format_rupiah(tot_aset_m)})</b>. Tidak ada selisih.</p></div>", unsafe_allow_html=True)
                else:
                    st.markdown(f"<div class='status-card-diff'><h4 style='color: #991B1B; margin:0;'>⚠️ STATUS: TERDAPAT SELISIH REKONSILIASI</h4><p style='color: #B91C1C; margin: 4px 0 0 0; font-size:12px;'>Ditemukan selisih sebesar <b>{format_rupiah(tot_selisih_m)}</b> antara Belanja Modal LRA ({format_rupiah(tot_lra_m)}) dan Aplikasi Aset ({format_rupiah(tot_aset_m)}).</p></div>", unsafe_allow_html=True)
            else:
                tot_lra_m = rekap_m['Nilai Realisasi'].sum()
                rows_html = "".join([
