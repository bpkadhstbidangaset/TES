...
with st.sidebar:
    ...
    st.info("📌 Master RAK otomatis terbaca.")
...
if f_lra:
    ...
    with tab1:
        ...
        if not df_pers_filtered.empty:
            ...
            st.dataframe(rekap_p_tampil[['Kode Rekening', 'Nama Rekening', 'Realisasi (Rp)']], use_container_width=True, hide_index=True)
    ...
    # TAB 2: BELANJA MODAL DENGAN FILTER KATEGORI / REKENING
    with tab2:
        if not df_modal_filtered.empty:
            # FITUR FILTER FLEKSIBEL BELANJA MODAL
            st.markdown("#### 🔍 Filter Pilihan Kategori Belanja Modal")
            opsi_kategori = sorted(df_modal_filtered['Uraian Kategori'].dropna().unique().tolist())
            kategori_terpilih = st.multiselect(
                "Centang/Pilih Kategori Aset yang ingin ditampilkan saja (Kosongkan jika ingin tampil semua):",
                options=opsi_kategori,
                default=[]
            )

            # Terapkan filter jika ada pilihan yang dipilih
            if kategori_terpilih:
                df_modal_final = df_modal_filtered[df_modal_filtered['Uraian Kategori'].isin(kategori_terpilih)].copy()
            else:
                df_modal_final = df_modal_filtered.copy()

            total_m = df_modal_final['Nilai Realisasi'].sum()
            st.markdown(f"""
            <div class="metric-box" style="background: linear-gradient(135deg, #4F46E5, #6366F1);">
                <div class="metric-title">🏢 Total Belanja Modal (Hasil Filter)</div>
                <div class="metric-value">{format_rupiah(total_m)}</div>
            </div>
            """, unsafe_allow_html=True)

            st.subheader("1. Rekapitulasi per Kategori & Rekening Belanja Modal")
            rekap_m = df_modal_final.groupby(['Kode Kategori', 'Uraian Kategori', 'Kode Rekening', 'Nama Rekening'])['Nilai Realisasi'].sum().reset_index()
            ...
            st.dataframe(rekap_m_tampil[['Kode Kategori', 'Uraian Kategori', 'Kode Rekening', 'Nama Rekening', 'Realisasi (Rp)']], use_container_width=True, hide_index=True)

            st.markdown("---")
            st.subheader("2. Rincian Dokumen Realisasi")
            df_detail_m = df_modal_final[[c for c in KOLOM_DETAIL_PILIHAN if c in df_modal_final.columns]].copy()
    ...
    st.info("👈 Silakan upload file **`LRA REALISASI JAN-JUNI30.xlsx`** pada panel kiri.")
